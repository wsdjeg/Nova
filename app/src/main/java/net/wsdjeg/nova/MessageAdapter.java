 ...
 * - TYPE_TIME_SEPARATOR: 时间轴节点（独立分隔项，不属于任何消息）
 *
 * 时间轴规则：
 * - 时间是消息流中的独立节点，不在每条消息下方显示时间
 * - 第一条消息之前始终显示一个时间节点
 * - 相邻可见项间隔超过 TIME_SEPARATOR_GAP_MS（约 5 分钟）时，
 *   在两条消息之间插入居中的时间节点
 * - 时间节点根据消息时间自动格式化（今天/昨天/本周/更早/跨年）
 *
 * 稳定标识 (StableKey):
...
public class MessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final float HEIGHT_SCALE_FACTOR = 1.0f; // 固定高度放大系数（不放大，每行按基础高度计算）

    /** 相邻消息间隔超过该值（5 分钟）时插入时间节点 */
    private static final long TIME_SEPARATOR_GAP_MS = 5L * 60 * 1000;

    private List<Message> messages;
    private List<Object> visibleItems;  // 可以是 Message / ToolCallItem / TimeSeparatorItem
    ...
    private static final int TYPE_TIME_SEPARATOR = 6;

    /**
     * 时间轴节点
     * 独立于用户/assistant/tool 消息的分隔项，不属于任何一条消息
     */
    public static class TimeSeparatorItem {
        public final long timestamp; // 毫秒

        public TimeSeparatorItem(long timestamp) {
            this.timestamp = timestamp;
        }

        public String getDisplayText() {
            return TimeUtils.formatTimelineTime(timestamp);
        }
    }
    ...
    private String computeStableKey(Object item) {
        if (item instanceof TimeSeparatorItem) {
            return "ts:" + ((TimeSeparatorItem) item).timestamp;
        }
    ...
    /**
     * 获取 visibleItem 的时间戳（毫秒）
     * 无有效时间戳时返回 0
     */
    private static long getTimestampOf(Object item) {
        if (item instanceof TimeSeparatorItem) {
            return ((TimeSeparatorItem) item).timestamp;
        }
        if (item instanceof ToolCallItem) {
            return ((ToolCallItem) item).getCreated() * 1000;
        }
     ...
     * 时间节点插入规则：
     * - 第一条可见消息之前始终插入一个时间节点
     * - 当前项与上一项间隔超过 TIME_SEPARATOR_GAP_MS 时，先插入时间节点再添加当前项
     * - 同一 assistant 消息拆分出的 content / tool_calls 共享时间戳，不会互相插入节点
     */
    private void buildVisibleItemsInto(List<Object> outItems, List<String> outKeys) {
        outItems.clear();
        outKeys.clear();
        Set<String> usedKeys = new HashSet<>();
        long lastItemTs = Long.MIN_VALUE; // 上一个可见项的时间戳
        ...
        for (int msgIdx = 0; msgIdx < messages.size(); msgIdx++) {
            ...
            // tool 消息：直接添加
            if (msg.isToolMessage()) {
                lastItemTs = maybeInsertTimeSeparator(outItems, outKeys, usedKeys,
                    msg.getTimestamp(), lastItemTs);
            ...
            if (msg.isAssistant() && msg.hasToolCalls()) {
                ...
                for (int i = 0; i < toolCalls.size(); i++) {
                    ...
                    lastItemTs = maybeInsertTimeSeparator(outItems, outKeys, usedKeys,
                        item.getCreated() * 1000, lastItemTs);
     ...
     * 按需在当前位置前插入时间节点
     *
     * @return 本项的时间戳（无有效时间戳时原样返回 lastItemTs）
     */
    private long maybeInsertTimeSeparator(List<Object> outItems, List<String> outKeys,
                                          Set<String> usedKeys, long ts, long lastItemTs) {
        if (ts <= 0) {
            return lastItemTs;
        }
        boolean isFirst = lastItemTs == Long.MIN_VALUE;
        if (isFirst || ts - lastItemTs > TIME_SEPARATOR_GAP_MS) {
            addItemWithKey(outItems, outKeys, usedKeys, new TimeSeparatorItem(ts));
        }
        return ts;
    }
    ...
    private boolean contentsEqual(Object a, Object b) {
        ...
        if (a instanceof TimeSeparatorItem) {
            TimeSeparatorItem ta = (TimeSeparatorItem) a;
            TimeSeparatorItem tb = (TimeSeparatorItem) b;
            // 同时比较时间戳和格式化文字（格式化结果依赖当前时间，如跨天后"今天"变"昨天"）
            return ta.timestamp == tb.timestamp
                && safeEq(ta.getDisplayText(), tb.getDisplayText());
        }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case TYPE_TIME_SEPARATOR:
                view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.item_time_separator, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof TimeSeparatorViewHolder) {
            bindTimeSeparatorViewHolder((TimeSeparatorViewHolder) holder, (TimeSeparatorItem) item);
    ...
    /**
     * 绑定时间轴节点
     * 时间是消息流中的独立分隔节点，不属于任何一条消息
     */
    private void bindTimeSeparatorViewHolder(TimeSeparatorViewHolder holder, TimeSeparatorItem item) {
        holder.timeText.setText(item.getDisplayText());
    }
    ...
    private int calculateHeightPx(int lines) {
        // 每行高度(像素) = 基础高度 × 1.0 × density
        float density = context.getResources().getDisplayMetrics().density;
        float lineSp = BASE_LINE_HEIGHT_SP * HEIGHT_SCALE_FACTOR; // 18sp × 1.0 = 18sp
    ...
    static class TimeSeparatorViewHolder extends RecyclerView.ViewHolder {
        ...
        TimeSeparatorViewHolder(View itemView) {
            super(itemView);
            timeText = itemView.findViewById(R.id.timeSeparatorText);
        }
    }
