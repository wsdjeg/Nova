...
- 🔄 **会话管理** - 查看、切换、创建、删除不同的对话会话，支持修改 provider/model/cwd/title
...
- 🛠️ **Tool Calls 显示** - 支持 AI 工具调用消息的完整显示
...
3. **会话设置**
   - 在聊天界面点击右上角菜单 → 「会话设置」
   - 查看/修改当前会话的 provider、model、cwd、标题
...
3. **加载历史** - 滚动到顶部自动加载更早的消息
4. **预览会话** - 点击菜单 → 「预览」在浏览器中打开 HTML 预览
