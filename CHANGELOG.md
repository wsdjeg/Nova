...
### feat (新功能)
- 468e5ac feat: use ZXing to generate QR code locally instead of downloading from URL
- d84623b feat: support WeChat re-login via DELETE /weixin/credentials API
- f04ba9a feat: WeChat login API update - support connected status and in-app QR display
- 8ee1125 feat: add bridge (integration) settings to session settings page
- fd937b5 feat: remove manual credential input from WeChat login
- 62ca7a5 feat: add WeChat login feature in Settings
- ccecf39 feat: add share functionality with upload confirmation
- b1c89df feat: move upload path setting to session settings, remove upload dialog
- ba971cb feat: render markdown in text selection dialog
- 8411611 feat: add default upload directory checkbox to upload dialog
- b2b1df5 feat: add upload progress dialog with progress bar and batch summary
- edd994a feat: support multiple image selection and batch upload
- d47eb17 feat: allow user to edit upload path before uploading image
- 0164d61 feat: add image upload to session working directory
- ffbbf7f feat: add "Select Text" option to message long-press menu for partial content copying
...
- 4ac5855 feat: update Java files to use string resources for i18n (part 2)
- 8d3c92f feat: update Java files to use string resources for i18n (part 1)
...
### fix (问题修复)
- cfa5bd7 fix: QR code generation fails silently due to narrow exception handling
- d9354ca fix: 不再提示配置未修改, 直接关闭即可
- f1c437d fix: make STATE_NORMAL/SENDING/LISTENING package-private for ChatVoiceHelper access
- 0a3fb60 fix: ShareActivity ApiClient constructor mismatch
- 7d55c34 fix: lambda variable capture and string resource format errors
- 4782ae6 fix: 文本选择对话框改用纯文本，避免代码块背景色覆盖选择高亮
- 2c4e28d fix: 移除 CHANGELOG.md 中过早写入的 v3.0.0 段落
- 30fd6ee fix: restore CHANGELOG.md that was accidentally deleted
...
- 002e8cc revert: 撤销 upload-dir 绝对路径修改，恢复本地存储方式
- d8282cb fix: upload-dir API 使用绝对路径，用 session cwd 拼接
- 7ebd65b fix: 上传图片默认路径改为本地存储，修复设置失败和单张图片显示问题
...
- 1ba3817 fix: use GitHub Secret for debug keystore instead of cache
- 08d8569 revert: use GitHub Secret for debug keystore instead of cache
...
- 97f3488 fix: remove unsupported setTimeout, add missing onFinalResult override
- ee46148 fix: remove @aar suffix from vosk-android dependency
...
- 6ba5740 fix: 修复 Markwon API 使用错误，使用 AbstractMarkwonPlugin 配置标题字号
...
### style (样式调整)
- 034eced style: reduce input EditText font size to 14sp to match message text
- 8d053dd style: reduce input box corner radius from 24dp to 8dp
- bbed5f4 style: compact message padding for tighter layout
...
### refactor (代码重构)
- a6ecec7 refactor: extract voice recognition logic from ChatActivity to ChatVoiceHelper
- f6921d7 refactor: extract image upload logic from ChatActivity to ChatUploadHelper
- 8baaf5f refactor: extract ApiClient inner classes to top-level (ChatMessage, ToolCall, Provider)
- 465995e refactor: use dialog for text selection instead of inline mode
...
### docs (文档更新)
- 68a3539 docs: update README and AGENTS.md with WeChat login, image upload, share, bridge settings and new file structure
- 8216df7 docs: add CI status badge to README
- efcad42 docs: reformat AGENTS.md in English
- a5bd379 docs: enforce CHANGELOG.md protection rule, remove improperly created CHANGELOG
- 1f7e37c docs: update README and CHANGELOG for v3.0 features
...
### chore (构建/工具)
- 044df9e chore: remove leftover fetched files and add to .gitignore
...
- bf6b4f5 ci: use GitHub Secret for debug keystore, no more base64 secret
